#ifndef light_h // light.h 중복 include 방지
#define light_h // light_h 정의
...
public: // 클래스 외부에서 접근 허용
	...
	void operator ~ (); // 빛의 크기를 출력하는 ~ 연산자 오버로딩
	Light& operator = (const Light& op2); // Light 객체를 매개변수로 하는 = 오버로딩
	Light operator -- (int x); // -- 후위 연산자 오버로딩
	bool operator == (const Light& op2) const; // Light 객체 두개의 빛의 크기를 비교하는 bool 타입 == 연산자 오버로딩 
};
